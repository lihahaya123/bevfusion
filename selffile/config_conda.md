...
## 1. 鍒涘缓 conda 鐜
```
conda create -n bevfusion python=3.10 -y
conda activate bevfusion

```
## 2. 瀹夎 PyTorch 2.x锛圕UDA 12.1/12.3锛?
```
# PyTorch 2.1.x 瀹樻柟鏀寔 CUDA 12.1锛屽湪 CUDA 12.3 涓嬪吋瀹?
pip install torch==2.1.2 torchvision==0.16.2 torchaudio==2.1.2 --index-url https://download.pytorch.org/whl/cu121
```
## 3. 瀹夎 OpenMPI 涓?mpi4py
```
# Linux
conda install -c conda-forge openmpi -y
pip install mpi4py
```
## 4. 瀹夎 mmcv 2.x 涓?mmdet 3.x
```
# mmcv 2.x 棰勭紪璇戝寘锛堟牴鎹?PyTorch 鍜?CUDA 鐗堟湰閫夋嫨锛?
...
```
pip install --prefer-binary tqdm torchpack nuscenes-devkit numba Pillow h5py
```

## 6. 淇敼 setup.py 閫傞厤 L40锛坰m_89锛?
闇€瑕佺紪杈?setup.py锛屽湪 make_cuda_ext 鍑芥暟涓坊鍔?sm_89 缂栬瘧鐩爣锛?

```
# 鍦?extra_compile_args["nvcc"] 鍒楄〃涓坊鍔狅細
...
extra_compile_args["nvcc"] = extra_args + [
    ...
    "-gencode=arch=compute_89,code=sm_89",  # 鏂板锛歀40 Ada Lovelace
]
