...
本文档说明如何从 Habitat-Sim/Replica 渲染标准 `robot_bev_dataset v4`
...
export OUTPUT_ROOT=/home/lihahaya/workspace/hikvision/bevfusion/data/replica_robot_bev_v4
```
...
```text
data/replica_robot_bev_v4/
```

如果切换过 BEV label source 或语义映射策略，需要使用新的空输出目录重新生成。
已有根目录中的 `bev_masks/*.npy` 不会因为代码更新而自动改变；旧 fingerprint
也不应通过 `--resume` 继续写入新策略数据。
...
Replica 适配器的 BEV 语义标签来自深度点携带的 semantic ID 投影；
`floor`/`carpet` 与 `wall`/`furniture`/`door`/`clutter` 一样都来自语义观测。
只有落在 canonical `x/y/z` 标签范围内的语义点才会写入 BEV label，其中
z 范围固定为 `[-0.5, 2.0]`。天花板及天花板附着物会被忽略。navmesh 只用于
采样和移动相机轨迹，不再用于
决定 `floor` 标签，也不向语义标签中混入机器人半径、可通行性或代价膨胀。
...
export PRODUCTION_ROOT=/path/to/output/replica_robot_bev_v4
...
恢复要求 dataset id、场景列表、split、帧数、传感器设置、轨迹相关
navmesh 设置、BEV label source、BEV 标签 `x/y/z` 范围、语义映射和
Habitat-Sim 版本保持一致。
不要手工拼接不同 fingerprint 的输出。

## 后续接入其他数据集

后续扩充其他数据集时，不要绕过 `robot_bev_dataset v4`。新的 source adapter
...
4. 六类语义映射：floor, carpet, wall, furniture, door, clutter
5. class_validity
6. observed mask
7. BEV label source
8. BEV 标签 `x/y/z` 范围
...
docker run --rm --gpus all \
  -v /path/to/bevfusion:/workspace \
  -v /mnt/datasets/replica_robot_bev_v4:/mnt/datasets/replica_robot_bev_v4 \
  -w /workspace \
  bevfusion:latest \
  torchpack dist-run -np 1 python tools/train.py \
    configs/robot_bev/seg/robotbev_camera_lidar_lss.yaml \
...
`map_pred/` 会先用 `--map-score` 过滤预测分数；没有任何类别大于阈值时显示为
background，有多个类别大于阈值时按 RobotBEV 可视化优先级显示。
`map_gt/` 仍按 GT mask 绘制。
...
| unobserved | 几何诊断图中的未观测区域 | 近黑色 | `(6, 6, 10)` |
| observed unlabeled | 几何诊断图中的已观测无类别区域 | 蓝灰色 | `(74, 76, 88)` |
| floor | 普通硬地面/地板表面 | 青色 | `(0, 180, 216)` |
| carpet | 地毯、地垫、毯子 | 蓝色 | `(0, 78, 255)` |
| wall | 墙体 | 黄色 | `(255, 238, 0)` |
| furniture | 大型/固定家具主体 | 橙红色 | `(255, 95, 31)` |
| door | 房门 | 绿色 | `(0, 200, 83)` |
| clutter | 其它杂物/无法明确归类区域 | 紫色 | `(186, 85, 211)` |

GT 和几何诊断图支持 multi-hot 标签。多个类别落在同一个 BEV 网格时，
可视化覆盖优先级固定为：

```text
door > furniture > wall > clutter > carpet > floor
```

也就是说，`door` 会覆盖同格的其它类别，`floor` 只在没有更高优先级类别时显示。
该规则同样用于 `tools/test.py --show-dir` 输出的 `map_gt/`。
`map_pred/` 会先用 `--map-score` 做置信度过滤；只有分数大于阈值的类别才参与
显示，然后在这些候选类别中按同一优先级选择一个显示。

Replica 数据生成中，六类 `map_gt` 标签均来自 semantic-depth 点投影，并且
只使用落在 canonical `x/y/z` 标签范围内的语义点。
`observed unlabeled` 表示该格子被观测射线覆盖，但没有任一语义点落入六类标签；
它不是 navmesh 可通行性或机器人代价地图。可通行/可清扫区域建议后续作为
独立预测头监督，不与当前 BEV 语义标签混合。
